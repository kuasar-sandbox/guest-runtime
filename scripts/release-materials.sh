...
release_materials_go_payload_allowed() {
  case "$1:$2" in
    runtime:bin/flatten-ctl|runtime:bin/sandbox-runtime.bundle:/sbin/init|runtime:bin/sandbox-runtime.bundle:/opt/sandbox-runtime/bin/envd ) return 0 ;;
...
release_materials_finish() {
  ...
  local toolchain_roots root_number
  while IFS= read -r toolchain; do
    ...
    while IFS= read -r toolchain_root; do
      directory="$RELEASE_MATERIALS_WORK/licenses-$toolchain-$root_number"
      ...
      # Every actual compiler root is checked. The identical authenticated
      # license tree needs only one copy per toolchain version in the bundle.
      if [ "$root_number" -eq 0 ]; then
