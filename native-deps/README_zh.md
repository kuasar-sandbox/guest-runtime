...
(autotools/kbuild/Go)与 Go 仓不同、上游发布节奏独立,
...
| `envd` | e2b-dev/infra 源码 tarball(tag `2026.22`) | `guest-runtime`(注入 `sandbox-runtime.bundle` 的 guest agent) |
| `versitygw`(可选) | versity/versitygw v1.5.0 | 按需用于本地/单节点 S3-compatible 文件存储网关,不包含在 `make build` 中 |

patched `cloud-hypervisor` 是 `sandbox-ctl` 的 VMM 运行件,由
`sandboxer/native-deps` 构建;`librocksdb`(`accelerator` 的 CGO 链接依赖)在
accelerator 仓内构建,不在此处。以上版本描述当前 Makefile 固定的输入,
不是一套独立版本策略。
...
make versitygw                 # 可选,不包含在 build 中
make build TARGET_ARCH=aarch64 # 交叉编译(CROSS_PREFIX 自动推导)

# 内核 patch 开发循环(在子 shell 中修改并提交,然后回到本目录)
