...
- **产物级跳过**:`bin/<arch>/` 下产物已存在时,erofs / vmlinux / envd 脚本直接退出
  (删除产物以强制重建);cloud-hypervisor 总是重跑 `cargo build`,由 cargo 增量缓存
  保证热构建秒级。
...
make cloud-hypervisor    # 打 patch + cargo build(冷 ~5-10 min,~200 crates;热秒级)
...
- 配置体系语义(两段拼接的契约、关键启用/禁用项)见
  [`sandbox-kernel.md`](sandbox-kernel.md) §2-§3。

### 2.3 cloud-hypervisor(`make cloud-hypervisor`)

= `ch-patches-apply` + `ch-build`,二者都进 `deps/build-cloud-hypervisor.sh`(STAGE
分阶段,§3):应用 `deps/ch-patches/*.patch` 后
`cargo build --release --locked --bin cloud-hypervisor` →
`bin/<arch>/cloud-hypervisor`。

- `--remap-path-prefix` 把源树映射为相对路径、registry 依赖映射到 `/cargo` 前缀,
  产物的 panic 消息与 DWARF 不泄漏构建机绝对路径(`$CH_SRC` / `$CARGO_HOME`);
  RUSTFLAGS 变化会失效 cargo 增量缓存,首次启用后是一次性的全量重编。
- host 构建依赖:rust 工具链(cargo / rustc);交叉编译另见 §4。
- patch 范围与语义见 [`cloud-hypervisor.md`](cloud-hypervisor.md) §1.2 / §3。
...
## 3. patch 开发循环(vmlinux / cloud-hypervisor 对称)

两个带补丁的产物共用同一套机制:`deps/build-{vmlinux,cloud-hypervisor}.sh` 是 STAGE
多阶段 dispatcher,Makefile 暴露同构的 `linux-*` / `ch-*` 目标:

| STAGE | make 目标(linux / ch) | 行为 |
|---|---|---|
| `fetch` | `linux-fetch` / `ch-fetch` | 解压 tarball → `git init` + 全量 import commit + 打 base tag(`linux-patches-base` / `ch-patches-base`);已有 base tag 则跳过;存在无 tag 的外来 git 树则拒绝(不覆盖 WIP) |
| `patches-apply` | `linux-patches-apply` / `ch-patches-apply` | `git am deps/{linux,ch}-patches/*.patch`;幂等与 sanity 语义见下 |
| `patches-format` | `linux-patches-format` / `ch-patches-format` | `git format-patch <base>..HEAD` 回写 `deps/*-patches/`(先清旧 `*.patch`) |
| `build` | `linux-build` / `ch-build` | 仅构建,不动 patch |

`linux-patches` / `ch-patches` 是各自 `*-patches-apply` 的别名。开发流(以 CH 为例,
内核侧把 `ch` 换成 `linux`、源树换成 `build/src/linux/`):

```bash
make ch-fetch                   # 一次性: 拉源码 + git tag ch-patches-base
cd build/src/cloud-hypervisor   # 改源码 + git commit(每个 patch 一个 commit)
make ch-patches-format          # 提取 base..HEAD 回 deps/ch-patches/*.patch
make cloud-hypervisor           # 重新 apply + build,验证可重复
```
...
- `HEAD = base + N` 且 N 等于 patch 数、commit subject 与 patch 文件逐一匹配:
  视为已应用,幂等跳过;
- 其他任何状态:报错并指引"先 `make {linux,ch}-patches-format` 保存 WIP,再
  `git reset --hard <base-tag>`,后重跑"。

补丁本身 arch-neutral:`deps/ch-patches/` 的 4 个 patch 不触 arch-specific 代码,
x86_64 与 aarch64 两个 cargo target 均可编译;`deps/linux-patches/` 仅触
...
| 取值 | 别名 | GOARCH | RUST_TARGET | KERNEL_ARCH |
|---|---|---|---|---|
| `x86_64` | `amd64` | `amd64` | `x86_64-unknown-linux-gnu` | `x86_64` |
...
```bash
make TARGET_ARCH=aarch64 build              # 全部四件
make TARGET_ARCH=aarch64 cloud-hypervisor   # 单件
```

### 4.2 工具链准备(x86_64 host → aarch64 为例,反向对称)

```bash
# C 工具链(erofs-utils configure/链接 + kbuild CROSS_COMPILE + cargo linker)
apt install gcc-aarch64-linux-gnu g++-aarch64-linux-gnu

# rust 目标 std 库(cloud-hypervisor)
...
缺失的工具链做前置探测(`${CROSS_PREFIX}gcc`、rustup target、`uuid-dev:<arch>`),
报错并给出安装指引,而不是在构建中途吐一墙链接错误。

## 5. WSL2 注意

WSL2 的 `/mnt/<drive>/`(DrvFs)上每个小文件有 5-10 倍 I/O 开销,而内核源树 ~85K
文件、CH 源树 ~50K 文件 + ~2 GiB cargo target,放在 DrvFs 上冷构建慢一个量级:
...
- [`cloud-hypervisor.md`](cloud-hypervisor.md) —— CH patch 范围、外部托管内存行为
  契约、启动协议与设备模型(随发布包)。
