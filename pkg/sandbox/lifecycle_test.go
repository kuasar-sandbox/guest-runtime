...
func TestWaitForCH_NoSignals(t *testing.T) {
	...
	doneCh <- nil
	err := waitForCHWithSignalEscalation(doneCh, sigCh, proc, 1234, "", time.Second, discardLogf)
	if err != nil {
...
func TestWaitForCH_SIGTERM_GracefulExit(t *testing.T) {
	...
	err := waitForCHWithSignalEscalation(doneCh, sigCh, proc, 1234, "", 5*time.Second, discardLogf)
	if err == nil {
...
func TestWaitForCH_SIGTERM_EscalatesToSIGKILL(t *testing.T) {
	...
	t0 := time.Now()
	sigCh <- syscall.SIGTERM
	err := waitForCHWithSignalEscalation(doneCh, sigCh, proc, 1234, "", grace, discardLogf)
...
func TestWaitForCH_DoubleSIGTERM_EscalatesImmediately(t *testing.T) {
	...
	_ = waitForCHWithSignalEscalation(doneCh, sigCh, proc, 1234, "", grace, discardLogf)
	elapsed := time.Since(t0)
