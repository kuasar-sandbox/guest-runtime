...
The current topology is governed by `launch.cgroup_control` in [sandboxer's cgroup implementation](https://github.com/kuasar-sandbox/sandboxer/blob/main/cmd/sandbox-init/cgroup.go). Sandbox-init remains in the guest-global root. Real `/sys/fs/cgroup/app` is always both the application's cgroup-namespace root and the recursive snapshot-freeze root:

- With `cgroup_control=false`, managed application processes start directly in real `/app` and see `0::/`. No delegated application subtree is promised; guest-root controller propagation is best effort.
- With `cgroup_control=true`, real `/app` remains empty and managed processes run in real `/app/init`, seen as `0::/init`. Sandbox-init strictly enables and verifies every advertised controller at the guest root and real `/app`. The first primary establishes the scoped cgroup namespace/mount and performs the one permitted move into `/init`; later primary restarts, plugins and native exec sessions are atomically cloned into the pinned final target and join that same namespace.
- Application-created groups, including envd's groups, remain under real `/app`; they are not siblings outside the freeze domain. Freezing `/app` covers the whole subtree. Namespace/setup/delegation failures do not silently weaken the requested `true` contract. See sandbox-init.md §3.2.1 for the full process and FD lifecycle.
...
The distinction is explicit: the kernel fragment contains CONFIG_VIRTIO_BALLOON=y, CONFIG_VIRTIO_BALLOON_FREE_PAGE_REPORTING=y and CONFIG_PAGE_REPORTING=y. Compiled capability does not mean the host advertises/enables the feature at runtime. No Kconfig value is changed by this documentation correction.

The alternative control path starts with a `mem_report` immediately after the cold-launch barrier, then reports MemAvailable and diagnostic fields every five seconds by default. The host obtains balloon target and `memory_actual_size` from CH `vm.info`; it neither derives Capacity from guest MemTotal nor requires the guest to report balloon current. The sandbox-local Budget controller adjusts target through `PUT /api/v1/vm.resize`. Shrink inflates at most 64 MiB per fresh report and waits for memory_actual_size to converge before another step. CH reclaims populated ranges with `fallocate(PUNCH_HOLE)` plus `madvise(DONTNEED)`. The platform's fourth VMM patch first probes file-backed ranges with `SEEK_DATA` and skips both operations for an already-sparse range, avoiding unnecessary UFFD_EVENT_REMOVE traffic; actual resident-data reclamation retains the original path. See [sandbox.md](https://github.com/kuasar-sandbox/sandboxer/blob/main/docs/sandbox.md) §9.3.

The current guest `/proc/meminfo` does not expose a `Balloon:` field despite CONFIG_VIRTIO_BALLOON=y. That field is neither guest ABI nor an optional source for Budget control; current always comes from CH memory_actual_size.
...
- [sandboxer/docs/sandbox.md](https://github.com/kuasar-sandbox/sandboxer/blob/main/docs/sandbox.md) §3.1 (`boot.kernel`) / §14.2 (platform ABI): kernel references and custom-kernel integration.
- [Native-build workflow](../native-deps/docs/build.md): make vmlinux, patch development and cross-compilation.
