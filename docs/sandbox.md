...
- `--memory-zone fd=3,uffd_socket=...`:patched CH 跳过 memfd_create,直接用
  ...
  vCPU 跑(详见 `native-deps/docs/cloud-hypervisor.md`)
...
`native-deps/docs/cloud-hypervisor.md` §3.4):无 madvise → 无同步握手,
...
- `size=0` boot 期 guest 看到 capacity 等额内存,balloon 尚未持有页。host 端
  ...
  `native-deps/docs/cloud-hypervisor.md` §3.4)
...
详见 `native-deps/docs/sandbox-kernel.md` §3、§5。
...
- [`sandbox-runtime.md`](sandbox-runtime.md) —— guest 内 sandbox-init 三阶段
  与 vsock 控制面协议
- `native-deps/docs/cloud-hypervisor.md` —— CH patches、命令行选项、
  外部托管内存契约
- `native-deps/docs/sandbox-kernel.md` —— guest kernel defconfig、平台
  ABI 边界
- `native-deps/docs/build.md` —— 原生依赖(mkfs.erofs / vmlinux /
