...
Thank you for contributing to `kuasar-sandbox/guest-runtime`.
...
```bash
git clone git@github.com:<your-login>/guest-runtime.git
cd guest-runtime
git remote add upstream git@github.com:kuasar-sandbox/guest-runtime.git
```
...
Open a pull request to the appropriate target: `main` for current development, or a supported `release/vMAJOR.MINOR.x` maintenance branch. For example, use `<your-login>/guest-runtime:feat/short-description` → `kuasar-sandbox/guest-runtime:main`. Enable **Allow edits from maintainers** when appropriate.
...
repo=kuasar-sandbox/guest-runtime
pr=123
